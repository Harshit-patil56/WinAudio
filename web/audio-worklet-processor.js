...
class WinAudioWorkletProcessor extends AudioWorkletProcessor {
    ...
    process(inputs, outputs) {
        ...
        // If buffer depth is higher than target, speed up subtly (1.01x - 1.045x) to catch up
        // If buffer depth is lower than target, slow down subtly (0.97x - 0.99x) to prevent underruns
        const bufferDiff = available - this.targetSamples;
        let targetSpeed = 1.000;

        if (bufferDiff > 96) { // More than ~2ms ahead of target
            targetSpeed = 1.000 + Math.min(0.045, (bufferDiff / (this.SAMPLE_RATE * 0.050)) * 0.045);
        } else if (bufferDiff < -96) { // Falling behind target
            targetSpeed = 1.000 - Math.min(0.030, (Math.abs(bufferDiff) / (this.SAMPLE_RATE * 0.050)) * 0.030);
        }

        // Smooth speed transitions (inertia prevents sudden pitch waver)
        this.currentSpeed += (targetSpeed - this.currentSpeed) * 0.05;

        // 4. Fractional Sample Interpolation Playback
        if (available >= 2) {
